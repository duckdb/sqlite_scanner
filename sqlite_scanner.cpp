...
static void AssertTypeMatches(int sqlite_column_type, int expected_type) {
	if (sqlite_column_type != expected_type) {
		throw Exception(ExceptionType::MISMATCH_TYPE, "Invalid type in column: column declared as " +
		                                                  SqliteTypeToString(expected_type) + ", found " +
		                                                  SqliteTypeToString(sqlite_column_type) + " instead");
	}
}
static void AssertTypeIsFloatOrInteger(int sqlite_column_type) {
	if (sqlite_column_type != SQLITE_FLOAT && sqlite_column_type != SQLITE_INTEGER) {
		throw Exception(ExceptionType::MISMATCH_TYPE, "Invalid type in column: expected float or integer, found " +
		                                                  SqliteTypeToString(sqlite_column_type) + " instead");
	}
}
		...
		while (true) {
			...
			for (idx_t col_idx = 0; col_idx < output.ColumnCount(); col_idx++) {
				...
				case LogicalTypeId::SMALLINT: {
					AssertTypeMatches(sqlite_column_type, SQLITE_INTEGER);
				...
				case LogicalTypeId::DOUBLE:
					AssertTypeIsFloatOrInteger(sqlite_column_type);
					FlatVector::GetData<double>(out_vec)[out_idx] = sqlite3_value_double(val);
					break;
				case LogicalTypeId::VARCHAR:
					AssertTypeMatches(sqlite_column_type, SQLITE_TEXT);
